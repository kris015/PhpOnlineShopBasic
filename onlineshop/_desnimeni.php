...
<div class="col-3">
    ...
    if(login()){
        if($_SESSION['status']=="Administrator"){
            echo "<div class='okvir'>
                    ...
                    <div>
                    <ul>
                        <li><a href='korisnici.php'>Rad sa korisnicima</a></li>
        ...
        echo "<div class='okvir'>
                    ...
                    <div>
                    <ul>
                        <li><a href='korpa.php'>Korpa (<span id='spanKorpa'>0</span>)</a></li>
    ...
    <div class="okvir">
        <div class="naslovDesni">Najnovije vesti</div>
        <div>
        <select class="form-select mr-3" aria-label="Default select example" id="sajt" name="sajt" onchange="posalji()">
                <option value="https://www.blic.rs/rss/IT" selected>Blic</option>
                <option value="https://www.kurir.rs/rss">Kurir</option>
                <option value="https://n1info.rs/feed/">N1</option>
        </select> <br>
        <div id="divVesti"></div>
        </div>
    </div>
...
<script>
    $(function(){
        $.post("ajax/ajax_korpa.php?akcija=prebrojProizvode", function(response){
            let odg=JSON.parse(response);
            $("#spanKorpa").html(odg.uspeh);
        })
        posalji();
    })
    function posalji(){
        $.post("ajax/ajax_vesti.php", {sajt:$("#sajt").val()}, function(response){
            $("#divVesti").html(response);
        })
    }
</script>